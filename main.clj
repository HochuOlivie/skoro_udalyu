...
(defn dijkstra [g src dst]                                    ; считаем само собой разумеющимся и очевидным
                                                              ; (aka библиотечный вызов, взятый с гитхаба)
...
(defn simplify-graph [graph]
  (let [g (get graph :forward)]           ; юзаем тока forward, backward депортируется в роcию
    (zipmap                               ; создаем мап 'начальный пункт':'конечный пункт':'цена проезда'
      (keys g)
      (map #(zipmap
              (keys %)
              (map
                (fn [param1] (if
                               (> (deref (param1 :tickets)) 0) ; если вообще есть билеты
                               (get param1 :price)             ; берем цену
                               9999)) (vals %))) (vals g)))))  ; если нет -- то ставим в качестве цены недостижимое число
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defn book-tickets
  ...
  (if (= from to)
    {:path '(), :price 0}
  (let [graph (simplify-graph route-map)       ; приводим в более перевариваемую форму (для дейкстры)
          path (dijkstra graph from to)]       ; за дело берется дейкстра (кратчайший путь по весам, если что)
        (try
          (dosync 														 ; запускаем в транзакции (атомарность, если что)
                                               ; далее будем апдейтить количество билетов (если чел какой-то билет забрал)
            (loop [p (second path)]            ; second path -- собсна наш кратчайший путь
              (if (= (count p) 2)              ; если переходим только по одному ребру в графе
                nil                            ; дальнейшие действия не имеют значения, прерываем loop
